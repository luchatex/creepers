										...
										<div class="form-group">
											...
											<div class="col-md-3">
												<select id="jobGroup" name="jobGroup"
													...
													<c:forEach items="${taskTypeList }" var="item">
														<option value="${item.value }" <c:if test="${item.value == jobGroup }">selected</c:if> >${item.name }</option>
													</c:forEach>
												</select>
											</div>
											...
											<div class="col-md-3">
												<select id="status" name="status" class="form-control">
													<option value='' >全部</option>
													<option value='NORMAL' <c:if test="${'NORMAL' eq status}">selected</c:if> >正常运行</option>
													<option value='PAUSED' <c:if test="${'PAUSED' eq status}">selected</c:if> >已暂停</option>
													<option value='COMPLETE' <c:if test="${'COMPLETE' eq status}">selected</c:if> >已完成</option>
													<option value='ERROR' <c:if test="${'ERROR' eq status}">selected</c:if> >已抛错</option>
													<option value='BLOCKED' <c:if test="${'BLOCKED' eq status}">selected</c:if> >已死锁</option>
													<option value='NONE' <c:if test="${'NONE' eq status}">selected</c:if> >已删除</option>
												</select>
											</div>
					...
					<div class="col-md-12">
						<div class="widget">
							<div class="widget-head">
								...
								<button class="btn btn-primary" title="新增" onclick="javascript:doAdd('${jobName}','${jobGroup }','${status }')">新增</button>
								</div>
								<div class="widget-icons pull-right">
									<a href="#" class="wminimize"><i class="icon-chevron-up"></i></a>
									<a href="#" class="wclose"><i class="icon-remove"></i></a>
											...
											<c:forEach items="${resultList.content}" var="result"
												...
												<tr>
													...
													<td>
														...
														<c:choose>
															<c:when test="${result.status == 'NONE' }">
																<button   class="btn btn-xs btn-primary disabled" title="更新"
																	onclick="javascript:doUpdate('${result.jobName}','${result.jobGroup}','${jobName}','${jobGroup }','${status }')">更新
																</button>
															</c:when>
															<c:otherwise>
														...
														<c:choose>
															<c:when test="${result.status == 'NONE' }">
																<button  class="btn btn-xs btn-info disabled" title="日志"
																	onclick="javascript:doShowLog('${result.jobName}')">日志
																</button>
															</c:when>
															<c:otherwise>
...
<script type="text/javascript">
	function doAdd(searchJobName,searchJobGroup,searchStatus) {
		window.location.href='${ctx}/job/toAdd?searchJobName='+searchJobName+"&searchJobGroup="+searchJobGroup+"&searchStatus="+searchStatus;
	}
	function doUpdate(jobName,jobGroup,searchJobName,searchJobGroup,searchStatus) {
		window.location.href='${ctx}/job/queryJob?jobName='+jobName+"&jobGroup="+jobGroup+"&searchJobName="+searchJobName+"&searchJobGroup="+searchJobGroup+"&searchStatus="+searchStatus;
	}
	...
	function doDelete(obj,jobName,jobGroup) {
		...
		$.ajax({
			cache : true,
			type : 'POST',
			url : '${ctx}/job/doDelete',
			data : {jobName:jobName,jobGroup:jobGroup},
			success : function(data) {
				if (data.message == "success") {
					$(obj).siblings().attr("disabled","disabled");
					var s = $(obj).prev().html();					
					if($.trim($(obj).prev().html())=="暂停"){
						$(obj).prev().remove();
					}
					$(obj).remove();
				}
	...
	function doShowLog(jobName) {
		window.location.href='${ctx}/job/queryLog?jobName='+jobName;
	}
	function doReSet(){
