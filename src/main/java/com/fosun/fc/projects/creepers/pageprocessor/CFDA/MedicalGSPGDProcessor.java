 ...
 * @see
 */
@Component("medicalGSPGDProcessor")
public class MedicalGSPGDProcessor implements PageProcessor {

    public static final String URL_PAGE_TEMPLATE = "http://www.sda.gov.cn/wbpp/generalsearch?sort=true&sortId=CTIME&record=20&columnid=CLID%7CBCID&relation=MUST%7CMUST&tableName=Region&colNum=2&qryNum=2&qryidstr=CLID%7CBCID&qryValue=cl0369%7C0019&record=20&mytarget=~blank&dateFormat=yyyy%C4%EAMM%D4%C2dd%C8%D5&titleLength=-1&subTitleFlag=0&classStr=%7C-1%7C-1%7CListColumnClass15%7CLawListSub15%7Clistnew15%7Clisttddate15%7Clistmore15%7Cdistance15%7Cclasstab15%7Cclasstd15%7CpageTdSTR15%7CpageTdSTR15%7CpageTd15%7CpageTdF15%7CpageTdE15%7CpageETd15%7CpageTdGO15%7CpageTdGOTB15%7CpageGOButton15%7CpageDatespan15%7Cpagestab15%7CpageGOText15&curPage=";

    private static final String REGEX_URL_PAGE = "http://www.sda.gov.cn/wbpp/generalsearch\\?sort=true&sortId=CTIME&record=20&columnid=CLID%7CBCID&relation=MUST%7CMUST&tableName=Region&colNum=2&qryNum=2&qryidstr=CLID%7CBCID&qryValue=cl0369%7C0019&record=20&mytarget=~blank&dateFormat=yyyy%C4%EAMM%D4%C2dd%C8%D5&titleLength=-1&subTitleFlag=0&classStr=%7C-1%7C-1%7CListColumnClass15%7CLawListSub15%7Clistnew15%7Clisttddate15%7Clistmore15%7Cdistance15%7Cclasstab15%7Cclasstd15%7CpageTdSTR15%7CpageTdSTR15%7CpageTd15%7CpageTdF15%7CpageTdE15%7CpageETd15%7CpageTdGO15%7CpageTdGOTB15%7CpageGOButton15%7CpageDatespan15%7Cpagestab15%7CpageGOText15&curPage=";

    private static final String REGEX_URL_DETAIL = "http://www.sda.gov.cn/WS01/CL0369/(.*).html";

    Logger logger = LoggerFactory.getLogger(this.getClass());

    private Site site;

    public MedicalGSPGDProcessor() {
        if (null == site) {
    ...
    public void process(Page page) {
        ...
        if (page.getUrl().toString().equals(param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL))) {
            ...
            for (Selectable td : tdList) {
                ...
                if (detailURL.contains("sda.gov.cn/CL0369/")) {
                    detailURL = detailURL.replace("sda.gov.cn/CL0369/", "sda.gov.cn/WS01/CL0369/");
                }
                logger.info("add detailURL:" + detailURL);
            ...
            if (CommonMethodUtils.isEmpty(trList)) {
                ...
                logger.debug("linksSelectable:" + linksSelectable.toString());
                logger.debug("is match:" + linksSelectable.regex("http://www.sda.gov.cn/directory/web/WS01/images/(.*).doc").match());
                if (linksSelectable.regex(param.getOrderUrl(BaseConstant.OrderUrlKey.DOWNLOAD_FILE_URL_1_REGEX)).match()) {
                    ...
                    if (i == titleRowNum) {
                        ...
                        for (int j = 0; j < tdList.size(); j++) {
                            ...
                            if ("企业名称".equals(CommonMethodUtils.matchesChineseValue(key[j]))) {
                                ...
                                logger.debug(CreepersConstant.TCreepersMedicalColumn.KEY.getValue() + ":" + value);
                            }
                            map.put(key[j], value);
                        }
                        list.add(map);
                ...
                Map<String, String> map = new HashMap<String, String>();
                map.put("province", "广州");
                TranslateMethodUtil.addCommonParamMapMedical(list, CreepersConstant.TCreepersMedicalColumn.TYPE.getValue(), BaseConstant.TaskListType.MEDICAL_GSP_LIST.getValue(),map);
            ...
            if (filePath.endsWith(BaseConstant.FileSuffix.DOC.getValue().toLowerCase())) {
                ...
                DocUtil.readExcelTableMedical(filePath, list);
            }
    ...
    public static void main(String[] args) {
        ...
        // "http://www.sda.gov.cn/WS01/CL0369/142620.html");
        ...
        Spider.create(new MedicalGSPGDProcessor()).setDownloader(new HttpRequestDownloader().setParam(param))
