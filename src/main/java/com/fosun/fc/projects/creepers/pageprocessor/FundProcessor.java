...
public class FundProcessor implements PageProcessor {
    ...
    private static final String REGEX_TAB_LOAN = "https://persons.shgjj.com/MainServlet?ID=5";
    ...
    private static final String CONTENT_LOAN_ACCOUNT_NO = "贷款账户号";
    private static final String CONTENT_LOAN_ACCOUNT_STATUS = "当前账户状态";
    private static final String CONTENT_LOAN_DT = "贷款开户日期";
    private static final String CONTENT_LOAN_PERIOD = "贷款期限";
    private static final String CONTENT_LOAN_REPAYMENT_METHOD = "还款方式";
    private static final String CONTENT_LOAN_AMOUNT = "贷款总额";
    private static final String CONTENT_LOAN_LEFT_AMOUNT = "贷款余额";
    private static final String CONTENT_LOAN_LAST_REPAYMENT_DT = "末次还款年月";
    ...
    public FundProcessor() {
        if (null == site) {
            site = Site.me().setDomain("persons.shgjj.com")
                    ...
                    .setAcceptStatCode(Sets.newHashSet(200, 302)).setRetryTimes(5).setSleepTime(78)
                    .setRetrySleepTime(81).setTimeOut(10000);
        }
    }
        ...
        try {
            ...
            if (param.getOrderUrl(BaseConstant.OrderUrlKey.INDEX_URL).equals(page.getUrl().toString())) {
                ...
                if (page.getHtml().xpath("//*[@id=\"myform\"]").match()) {
                    ...
                    page.setSkip(true);
                } else if (page.getHtml().xpath(XPATH_ERROR_TIP + XPATH_ALLTEXT).toString().contains("用户名")) {
                    creepersListFundServiceImpl.updateStatus(param.getLoginName(), "3");
                    page.setSkip(true);
                } else {
                    param.setErrorInfo(page.getHtml().xpath(XPATH_ERROR_TIP + XPATH_ALLTEXT).toString());
                    param.setErrorPath(getClass().toString());
                ...
                } else if (REGEX_TAB_LOAN.equals(page.getUrl().toString())) {
                    logger.debug("=====>>>住房公积金贷款信息爬取开始!");
                    if (page.getHtml().xpath(XPATH_FUND_TABLE_BY_COLOR + XPATH_TR).nodes().size() > 0)
                        methodFundLoan(page);
                    logger.debug("=====>>>住房公积金贷款信息爬取结束!");
                } else {
                    page.setSkip(true);
                }
            } else {
     ...
     */
    private void methodFundBasic(Page page) throws Exception {
        ResultItems resultItems = page.getResultItems();
        CreepersLoginParamDTO param = resultItems.get(BaseConstant.PARAM_DTO_KEY);
        ...
        for (Selectable tdSel : tdList) {
            ...
            String currentContent = tdSel.xpath(XPATH_ALLTEXT).toString().replace(UTFSpace, "").trim();
            ...
            if (CONTENT_NAME.equals(currentContent)) {
                ...
                // 姓名
                map.put(CreepersConstant.TCreepersFundBasicColumn.NAME.getValue(), currentContent.substring(0,
                        currentContent.indexOf(">") > 0 ? currentContent.indexOf(">") : currentContent.length()));
                logger.info(CreepersConstant.TCreepersFundBasicColumn.NAME.getValue() + SPILT_COLON
                        + currentContent.substring(0, currentContent.indexOf(">") > 0 ? currentContent.indexOf(">")
                ...
                lastContent = CONTENT_EMPTY;
                // 账户余额
                map.put(CreepersConstant.TCreepersFundBasicColumn.SUM_AMOUNT.getValue(), currentContent.substring(0,
                        currentContent.indexOf("元") > 0 ? currentContent.indexOf("元") : currentContent.length()));
                logger.info(CreepersConstant.TCreepersFundBasicColumn.SUM_AMOUNT.getValue() + SPILT_COLON
                        + currentContent.substring(0, currentContent.indexOf("元") > 0 ? currentContent.indexOf("元")
                                : currentContent.length()));
                ...
                lastContent = CONTENT_EMPTY;
                // 月缴存额
                map.put(CreepersConstant.TCreepersFundBasicColumn.MONTHLY_AMOUNT.getValue(), currentContent.substring(0,
                        currentContent.indexOf("元") > 0 ? currentContent.indexOf("元") : currentContent.length()));
                ...
                lastContent = CONTENT_EMPTY;
                // 绑定手机号
                map.put(CreepersConstant.TCreepersFundBasicColumn.MOBILE.getValue(), currentContent.substring(0,
                        currentContent.indexOf("【") > 0 ? currentContent.indexOf("【") : currentContent.length()));
                ...
                lastContent = CONTENT_EMPTY;
                // 实名认证状态
                map.put(CreepersConstant.TCreepersFundBasicColumn.CERTIFICATE_STATUS.getValue(),
                        currentContent.substring(0, currentContent.indexOf(" ") > 0 ? currentContent.indexOf(" ")
                                : currentContent.length()));
        ...
        page.putField(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_BASIC.getMapKey(), map);
    }
    ...
    private void methodFundBasicDetail(Page page) throws Exception {
        ...
        page.putField(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_BASIC_DETAIL.getMapKey(), mapList);
    }
    ...
    private void methodFundExtra(Page page) {
        ...
        for (Selectable tdSel : tdList) {
            ...
            if (CONTENT_NAME.equals(currentContent)) {
                ...
                lastContent = CONTENT_EMPTY;
                // 姓名
                map.put(CreepersConstant.TCreepersFundExtraColumn.NAME.getValue(), currentContent.substring(0,
                        currentContent.indexOf(">") > 0 ? currentContent.indexOf(">") : currentContent.length()));
                ...
                lastContent = CONTENT_EMPTY;
                // 账号余额
                map.put(CreepersConstant.TCreepersFundExtraColumn.SUM_AMOUNT.getValue(), currentContent.substring(0,
                        currentContent.indexOf("元") > 0 ? currentContent.indexOf("元") : currentContent.length()));
                ...
                lastContent = CONTENT_EMPTY;
                // 月缴存额
                map.put(CreepersConstant.TCreepersFundExtraColumn.MONTHLY_AMOUNT.getValue(), currentContent.substring(0,
                        currentContent.indexOf("元") > 0 ? currentContent.indexOf("元") : currentContent.length()));
        ...
        page.putField(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_EXTRA.getMapKey(), map);
    }
    ...
    private void methodFundExtraDetail(Page page) throws Exception {
        ...
        page.putField(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_EXTRA_DETAIL.getMapKey(), mapList);
    }

    /**
     * 
     * <p>
     * description: 住房公积金贷款信息爬取
     * </p>
     * 
     * @param page
     * @author Pengyk
     ...
     * @see 2016-7-12 13:04:31
     */
    private void methodFundLoan(Page page) throws Exception {
        ResultItems resultItems = page.getResultItems();
        CreepersLoginParamDTO param = resultItems.get(BaseConstant.PARAM_DTO_KEY);
        List<Selectable> tdList = page.getHtml().xpath(XPATH_FUND_TABLE_BY_COLOR + XPATH_TBODY + XPATH_TD).nodes();
        Map<String, String> map = new HashMap<String, String>();
        ...
        for (Selectable tdSel : tdList) {
            ...
            String currentContent = tdSel.xpath(XPATH_ALLTEXT).toString().replace(UTFSpace, "").trim();
            logger.info("currentContent:" + currentContent);
            if (currentContent.contains(CONTENT_LOAN_ACCOUNT_NO)) {
                lastContent = CONTENT_LOAN_ACCOUNT_NO;
                continue;
            } else if (CONTENT_LOAN_ACCOUNT_NO.equals(lastContent)) {
                lastContent = CONTENT_EMPTY;
                // 贷款账户号
                map.put(CreepersConstant.TCreepersFundLoansColumn.LOAN_ACCOUNT_NO.getValue(), currentContent);
                logger.info(CreepersConstant.TCreepersFundLoansColumn.LOAN_ACCOUNT_NO.getValue() + SPILT_COLON
                        + currentContent);
            } else if (CONTENT_LOAN_ACCOUNT_STATUS.equals(currentContent)) {
                lastContent = CONTENT_LOAN_ACCOUNT_STATUS;
                continue;
            } else if (CONTENT_LOAN_ACCOUNT_STATUS.equals(lastContent)) {
                lastContent = CONTENT_EMPTY;
                // 当前账户状态
                map.put(CreepersConstant.TCreepersFundLoansColumn.LOAN_ACCOUNT_STATUS.getValue(), currentContent);
                logger.info(CreepersConstant.TCreepersFundLoansColumn.LOAN_ACCOUNT_STATUS.getValue() + SPILT_COLON
                        + currentContent);
            } else if (CONTENT_LOAN_DT.equals(currentContent)) {
                lastContent = CONTENT_LOAN_DT;
                continue;
            } else if (CONTENT_LOAN_DT.equals(lastContent)) {
                lastContent = CONTENT_EMPTY;
                // 贷款开户日期
                if (StringUtils.isNotBlank(currentContent))
                    map.put(CreepersConstant.TCreepersFundLoansColumn.LOAN_DT.getValue(), currentContent);
                logger.info(
                        CreepersConstant.TCreepersFundLoansColumn.LOAN_DT.getValue() + SPILT_COLON + currentContent);
            } else if (CONTENT_LOAN_PERIOD.equals(currentContent)) {
                lastContent = CONTENT_LOAN_PERIOD;
                continue;
            } else if (CONTENT_LOAN_PERIOD.equals(lastContent)) {
                lastContent = CONTENT_EMPTY;
                // 贷款期限
                map.put(CreepersConstant.TCreepersFundLoansColumn.PERIOD.getValue(), currentContent);
                logger.info(CreepersConstant.TCreepersFundLoansColumn.PERIOD.getValue() + SPILT_COLON + currentContent);
            } else if (CONTENT_LOAN_REPAYMENT_METHOD.equals(currentContent)) {
                lastContent = CONTENT_LOAN_REPAYMENT_METHOD;
                continue;
            } else if (CONTENT_LOAN_REPAYMENT_METHOD.equals(lastContent)) {
                lastContent = CONTENT_EMPTY;
                // 还款方式
                map.put(CreepersConstant.TCreepersFundLoansColumn.REPAYMENT_METHOD.getValue(), currentContent);
                logger.info(CreepersConstant.TCreepersFundLoansColumn.REPAYMENT_METHOD.getValue() + SPILT_COLON
                        + currentContent);
            } else if (CONTENT_LOAN_AMOUNT.equals(currentContent)) {
                lastContent = CONTENT_LOAN_AMOUNT;
                continue;
            } else if (CONTENT_LOAN_AMOUNT.equals(lastContent)) {
                lastContent = CONTENT_EMPTY;
                // 贷款总额
                map.put(CreepersConstant.TCreepersFundLoansColumn.LOAN_AMOUNT.getValue(), currentContent.substring(0,
                        currentContent.indexOf("元") > 0 ? currentContent.indexOf("元") : currentContent.length()));
                logger.info(CreepersConstant.TCreepersFundLoansColumn.LOAN_AMOUNT.getValue() + SPILT_COLON
                        + currentContent.substring(0, currentContent.indexOf("元") > 0 ? currentContent.indexOf("元")
                                : currentContent.length()));
            } else if (CONTENT_LOAN_LEFT_AMOUNT.equals(currentContent)) {
                lastContent = CONTENT_LOAN_LEFT_AMOUNT;
                continue;
            } else if (CONTENT_LOAN_LEFT_AMOUNT.equals(lastContent)) {
                lastContent = CONTENT_EMPTY;
                // 贷款余额
                map.put(CreepersConstant.TCreepersFundLoansColumn.LEFT_AMOUNT.getValue(), currentContent.substring(0,
                        currentContent.indexOf("元") > 0 ? currentContent.indexOf("元") : currentContent.length()));
                logger.info(CreepersConstant.TCreepersFundLoansColumn.LEFT_AMOUNT.getValue() + SPILT_COLON
                        + currentContent.substring(0, currentContent.indexOf("元") > 0 ? currentContent.indexOf("元")
                                : currentContent.length()));
            } else if (CONTENT_LOAN_LAST_REPAYMENT_DT.equals(currentContent)) {
                lastContent = CONTENT_LOAN_LAST_REPAYMENT_DT;
                continue;
            } else if (CONTENT_LOAN_LAST_REPAYMENT_DT.equals(lastContent)) {
                lastContent = CONTENT_EMPTY;
                // 末次还款年月
                if (StringUtils.isNotBlank(currentContent))
                    map.put(CreepersConstant.TCreepersFundLoansColumn.LAST_REPAYMENT_DT.getValue(), currentContent);
                logger.info(CreepersConstant.TCreepersFundLoansColumn.LAST_REPAYMENT_DT.getValue() + SPILT_COLON
                        + currentContent);
            }
        }
        page.putField(CreepersConstant.TableNamesFund.T_CREEPERS_FUND_LOANS.getMapKey(), map);
    }
    ...
    public static void main(String[] args) {
        ...
        param.setLoginName("178413");
        param.setPassword("bxy178413");
        ...
        param.putTargetUrlList("https://persons.shgjj.com/MainServlet?ID=5");
        param.putNameValuePair("SUBMIT.x", "24");
        param.putNameValuePair("SUBMIT.y", "8");
        param.putNameValuePair("password_md5", "5EA52137ABE56EBC101EE3D0F87A496E");
        param.putNameValuePair("ID", "0");
