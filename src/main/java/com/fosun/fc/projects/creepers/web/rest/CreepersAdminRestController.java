...
public class CreepersAdminRestController {

    @Autowired
    private ICreepersListService creepersListServiceImpl;

    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping(value = "/adminSaction/query", method = RequestMethod.POST, produces = MediaTypes.JSON_UTF_8)
    public ResponseEntity<JsonResult<Map>> queryByMerName(@RequestBody String inputJson, HttpServletRequest request,
            HttpServletResponse response) {
        JsonResult jsonResult = null;
        CreepersAdminDTO dto = JSONObject.parseObject(inputJson, CreepersAdminDTO.class);
        if (StringUtils.isNotBlank(dto.getKey())) {
            jsonResult = creepersListServiceImpl.processByMerName(BaseConstant.TaskListType.ADMIN_SACTION_LIST.getValue(), dto.getKey());
