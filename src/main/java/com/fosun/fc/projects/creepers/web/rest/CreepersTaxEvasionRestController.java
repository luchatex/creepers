...
public class CreepersTaxEvasionRestController {

    @Autowired
    private ICreepersListService creepersListServiceImpl;

    @SuppressWarnings({ "rawtypes", "unchecked" })
    @RequestMapping(value = "/taxEvasion/query", method = RequestMethod.POST, produces = MediaTypes.JSON_UTF_8)
    public ResponseEntity<JsonResult<Map>> queryByMerName(@RequestBody String inputJson, HttpServletRequest request,
            HttpServletResponse response) {
        JsonResult jsonResult = null;
        CreepersTaxEvasionDTO param = JSONObject.parseObject(inputJson, CreepersTaxEvasionDTO.class);
        if (StringUtils.isNotBlank(param.getName())) {
            jsonResult = creepersListServiceImpl.processByMerName(BaseConstant.TaskListType.TAX_EVASION_LIST.getValue(),
                    param.getName());
