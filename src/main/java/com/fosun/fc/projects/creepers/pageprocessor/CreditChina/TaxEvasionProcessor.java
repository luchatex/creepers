...
public class TaxEvasionProcessor extends BaseCreepersListProcessor implements PageProcessor {
            ...
            if (page.getUrl().regex(BASE_URL + REGEX_URL_CREDIT_SEARCH).match()) {
                if (JsonValidator.validate(page.getJson().toString())) {
                    ...
                    for (int i = 0; i < jsonArr.size(); i++) {
                        ...
                        logger.info(CreepersConstant.TCreepersShixinAllColumn.PROVINCE.getValue() + SYMBOL_COLON_EN
                                + province);
                    ...
                    page.putField(CreepersConstant.TableNamesCreditChina.T_CREEPERS_TAX_EVASION.getMapKey(),
                            tCreepersTaxEvasion);
                    int totalPageCount = null == json.getJSONObject(JSON_KEY_RESULT)
                            .getString(JSON_KEY_TOTAL_PAGE_COUNT) ? 0
                                    : Integer.parseInt(
                                            json.getJSONObject(JSON_KEY_RESULT).getString(JSON_KEY_TOTAL_PAGE_COUNT));
                        ...
                        for (int j = 1; j <= threadNum; j++) {
                            if (pageNo + j <= totalPageCount) {
                                Request request = new Request(
                                        BASE_URL + ACCESS_URL_CREDIT_SEARCH + new Date().getTime());
    ...
    public void getNegativeInfo(Page page, Selectable divSel) {
        ...
        for (Selectable ulSel : ulList) {
            ...
            if("税务局".equals(map.get(CreepersConstant.TCreepersTaxEvasionDetailColumn.SOURCE.getValue())))
    ...
    public Site getSite() {
        if (null == site) {
            site = Site.me().setDomain(BASE_URL).setUserAgent(USER_AGENT).setCycleRetryTimes(6).setRetryTimes(2).setTimeOut(60000).setSleepTime(7000).setCharset("UTF-8");
        }
        return site;
    }
