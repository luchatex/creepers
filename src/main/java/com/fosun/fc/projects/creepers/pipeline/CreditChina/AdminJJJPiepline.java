 ...
 * 
 * @author Luxin
 * @since 2017-3-20 14:09:12
 * @see
 */
@Component
public class AdminJJJPiepline extends BasePipeline {

    private Logger logger = LoggerFactory.getLogger(getClass());

    private final String TYPE_AREA = "京津冀";
    ...
    public AdminJJJPiepline() {
        setPath("/data/webmagic/");
    }

    public AdminJJJPiepline(String path) {
        setPath(path);
    }
    ...
    public void process(ResultItems resultItems, Task task) {
        logger.info("step:  ======>>  entry AdminJJJPiepline process!");
        // 数据入库
        ...
        try {
            if (!CommonMethodUtils.isEmpty(resultList)) {
                ...
                for (Map<String, String> resultMap : resultList) {
                    ...
                    entity.setMemo(BaseCode.encryptMD5To16(resultMap.toString().getBytes()));
                ...
                logger.info("step:  ======>>  creepersAdminJJJServiceImpl.saveEntity START!");
                //保存
                creepersAdminJJJServiceImpl.saveOrUpdate(entityList);
                logger.info("step:  ======>>  creepersAdminJJJServiceImpl.saveEntity SUCCEED!");
            ...
            e.printStackTrace();
            logger.error("step:  ======>>  AdminJJJPipline  save FALSE!!!");
        ...
        logger.info("step:  ======>>  exit AdminJJJPipline process!");
    }
}
