...
public class CreepersExceptionHandleServiceImpl implements ICreepersExceptionHandleService {
    ...
    public void handleJobExceptionAndPrintLog(CreepersParamDTO param) {
        ...
        String[] ccMails = new String[] { "pengyk@fosun.com", "maxin@fosun.com", "Luxin@fosun.com" };
        String ip="";
        try {
        ...
        int startIndex = 0;
        if (param.getErrorInfo().contains("write DB"))
            startIndex = 9;
        if (param.getErrorInfo().contains(":")) {
            exceptionName = param.getErrorInfo().substring(startIndex, param.getErrorInfo().indexOf(":"));
        }else {
            exceptionName = param.getErrorInfo().substring(startIndex);
        }
        frequency.frequency("com.fosun.fc.projects.creepers.service.impl.SimpleMailServiceImpl", "sendNotificationMail",
                classes, parameterValues, "mail.maxTimeInterval", "mail.maxTimes", param.getTaskType(),
                exceptionName);
    }
}
