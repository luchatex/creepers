 ...
 */
@Component
public class AdminZhejiangPiepline extends BasePipeline {

    private Logger logger = LoggerFactory.getLogger(getClass());

    private final String TYPE_AREA = "浙江";
    ...
    public AdminZhejiangPiepline() {
        setPath("/data/webmagic/");
    }

    public AdminZhejiangPiepline(String path) {
        setPath(path);
    }
    ...
    public void process(ResultItems resultItems, Task task) {
        ...
        try {
            if (!CommonMethodUtils.isEmpty(resultList)) {
                ...
                logger.info("step:  ======>>  creepersAdminZhejiangServiceImpl.saveEntity START!");
                //保存
                creepersAdminZhejiangServiceImpl.saveOrUpdate(entityList);
                logger.info("step:  ======>>  creepersAdminZhejiangServiceImpl.saveEntity SUCCEED!");
            ...
            e.printStackTrace();
            logger.error("step:  ======>>  AdminZhejiangPipline  save FALSE!!!");
