...
public class SupremeCourtDishonestyBlackListProcessor extends BaseCreepersListProcessor implements PageProcessor {
    ...
    private static final Set<String> COURT_DIS_INFO_COLUMN_NAME = Sets.newHashSet("企业名称", "失信被执行人名称");
    ...
    public SupremeCourtDishonestyBlackListProcessor() {
        site = Site.me().setDomain(BASE_URL).setUserAgent(USER_AGENT).setCycleRetryTimes(6).setRetryTimes(2)
    ...
    public void process(Page page) {
        ...
        try {
            if (page.getUrl().regex(BASE_URL + REGEX_URL_CREDIT_SEARCH).match()) {
                if (JsonValidator.validate(page.getJson().toString())) {
                    ...
                    for (int i = 0; i < jsonArr.size(); i++) {
                        ...
                        logger.info(CreepersConstant.TCreepersCourtDishonestColumn.NAME.getValue() + SYMBOL_COLON_EN
                                + name);
                        map.put(CreepersConstant.TCreepersCourtDishonestColumn.CODE.getValue(), code);
                        logger.info(CreepersConstant.TCreepersCourtDishonestColumn.CODE.getValue() + SYMBOL_COLON_EN
                                + code);
                        ...
                        page.addTargetRequest(BASE_URL + ACCESS_URL_DETAIL + URL_PARAM_OBJECT_TYPE + SYMBOL_EQ
                                + objectType + SYMBOL_AND + URL_PARAM_ENCRY_STR + SYMBOL_EQ + encryStr);
            ...
            param.setErrorInfo("SupremeCourtDishonestyBlackListProcessor process error:" + errorInfo);
    ...
    public void getSupremeCourtDisInfo(Page page, Selectable divSel) {
        ...
        for (Selectable ulSel : ulList) {
            ...
            for (Selectable liSel : liList) {
                ...
                if (COURT_DIS_INFO_COLUMN_NAME.contains(name)) {
            ...
            if ("最高法".equals(map.get(CreepersConstant.TCreepersCourtDisInfoColumn.SOURCE.getValue())))
    ...
    public static void main(String[] args) throws IOException {
        ...
        String url = "http://www.creditchina.gov.cn/shiXinRen?t=" + new Date().getTime();
